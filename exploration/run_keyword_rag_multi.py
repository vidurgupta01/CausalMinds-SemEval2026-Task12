#!/usr/bin/env python3
"""
Keyword RAG with multi-cause awareness.
Explicitly asks model to consider ALL potential causes.
"""
...
def run_with_multi_awareness(question, context):
    """Prompt that explicitly asks for ALL causes."""
    prompt = f"""Based on the context, identify ALL options that are direct CAUSES of this event.
IMPORTANT: Events often have MULTIPLE causes. Check EACH option carefully.

Context: {context if context else "No additional context."}

Event: {question['target_event']}

For each option, ask: Did this happen BEFORE the event AND directly cause it?
A. {question['option_A']}
B. {question['option_B']}
C. {question['option_C']}
D. {question['option_D']}

List ALL letters that are direct causes (comma-separated):"""
...
def main():
    ...
    print(f"Running KEYWORD RAG (MULTI-AWARE) on {len(questions)} samples\n")
    ...
    for q in tqdm(questions):
        ...
        response = run_with_multi_awareness(q, context)
    ...
    print(f"\n{'='*60}")
    print(f"KEYWORD RAG (MULTI-AWARE) RESULTS")
    print(f"{'='*60}")
    ...
    print(f"Baseline: 0.6825, Keyword RAG: 0.7600")
    print(f"{'='*60}")

    with open(f"experiments/keyword_rag_multi_{args.split}_results.json", "w") as f:
