#!/usr/bin/env python3
"""
Keyword RAG v2: More sentences + simpler prompt.
"""
...
def extract_relevant_context(question, docs, max_sentences=8):
    """Use more sentences for context."""
    if not docs:
        return ""
...
def run_with_context(question, context):
    """Simpler prompt."""
    prompt = f"""Context: {context if context else "No context."}
...
def main():
    ...
    print(f"Running KEYWORD RAG v2 on {len(questions)} samples\n")
    ...
    print(f"\n{'='*60}")
    print(f"KEYWORD RAG v2 RESULTS")
    print(f"{'='*60}")
    ...
    with open(f"experiments/keyword_rag_v2_{args.split}_results.json", "w") as f:
